...
void WifiStation::HandleScanResult() {
    ...
    for (int i = 0; i < ap_num; i++) {
        ...
        //在 ssid_list 容器中查找第一个与 ap_record.ssid 相同的 SSID 名称，并返回其迭代器
    ...
    if (connect_queue_.empty()) {
        ...
        ContinueScan = true;
        esp_timer_start_once(timer_handle_, 0.5 * 1000000);
        return;
    }
...
void WifiStation::StartConnect() {
    ...
    for(size_t i = 0; i < connect_queue_.size(); i++) {
        if (connect_queue_[i].rssi > best_rssi) {
            best_rssi = connect_queue_[i].rssi;
            best_idx = i;
        }
    }
    auto ap_record = connect_queue_[best_idx];
    connect_queue_.erase(connect_queue_.begin() + best_idx);
    if(GetSsid() == ap_record.ssid && IsConnected()){
        ESP_LOGI(TAG, "No Wifi Better Than %s, Keep Current Wifi", ap_record.ssid.c_str());
        return;
    }
    ...
    if(IsConnected())
    {
        SwitchWIfi = true;
        esp_wifi_disconnect();
        ESP_LOGI(TAG, "Disconnect from current Wifi %s", GetSsid().c_str());
    }

    ssid_ = ap_record.ssid;
    password_ = ap_record.password;
    ESP_LOGI(TAG, "Found Better Wifi: %s, RSSI: %d, Switch WiFi", ssid_.c_str(), ap_record.rssi);
    ...
    ESP_LOGI(TAG, "Connecting to %s", ssid_.c_str());
...
int8_t WifiStation::GetRssi() {
    ...
    if (!IsConnected()) {
        ESP_LOGW(TAG, "GetRssi failed: not connected");
        return -127; // 约定值：未连接
    }
    ...
    ESP_LOGI(TAG, "GetRssi failed: %s", esp_err_to_name(err));
    return -127; // 约定值：未连接
}
...
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        if(this_->SwitchWIfi)
        {
            ESP_LOGI(TAG, "Switch to new Wifi :%s", this_->ssid_.c_str());
            this_->SwitchWIfi = false;
            return;
        }
