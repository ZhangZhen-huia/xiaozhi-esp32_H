...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_size(main_screen_, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_radius(main_screen_, 0, 0);
    lv_obj_set_style_bg_opa(main_screen_, 255, 0);
    lv_obj_set_style_border_width(main_screen_, 0, 0); // 设置边框宽度为0
    lv_obj_set_style_pad_all(main_screen_, 0, 0); // 设置内边距为0
    lv_obj_set_pos(main_screen_, 0, 0); // 设置位置为(0, 0)
    lv_obj_set_style_text_font(main_screen_, text_font, 0);
    lv_obj_set_style_text_color(main_screen_, lvgl_theme->text_color(), 0);
    lv_obj_set_style_bg_color(main_screen_, lvgl_theme->background_color(), 0);
    current_screen_ = main_screen_;
    ...
    low_battery_popup_ = lv_obj_create(main_screen_);
    lv_obj_set_scrollbar_mode(low_battery_popup_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_size(low_battery_popup_, LV_HOR_RES * 0.9, text_font->line_height * 2);
    lv_obj_align(low_battery_popup_, LV_ALIGN_BOTTOM_MID, 0, -lvgl_theme->spacing(4));
    lv_obj_set_style_bg_color(low_battery_popup_, lvgl_theme->low_battery_color(), 0);
