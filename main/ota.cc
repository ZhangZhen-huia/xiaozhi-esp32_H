...
bool Ota::CheckVersion() {
    auto& board = Board::GetInstance();
    //获取当前正在运行的应用程序的描述信息头（esp_app_desc_t）
    //里面包含版本号、编译时间、ELF SHA256 等关键字段
    ...
    //http://a.cn 都有11个字符了，所以真正的URL不可能小于10个字符    
    if (url.length() < 10) {
    ...
    if (cJSON_IsObject(mqtt)) {
        ...
        ESP_LOGW(TAG, " mqtt section found !");
    ...
    if (cJSON_IsObject(websocket)) {
        ...
        ESP_LOGW(TAG, "websocket section found!");
