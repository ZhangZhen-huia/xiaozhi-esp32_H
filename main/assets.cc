...
bool Assets::InitializePartition() {
    ...
    //在 Flash 分区表里找到名字叫 assets 的第一个分区
    //不论是什么类型的（不论是App还是Data）
    ...
    //mmap（memory map，内存映射）是 把 文件/设备/Flash 分区 
    //整块“挂”到进程虚拟地址空间 的一种机制
    //挂成功后，CPU 像访问 RAM 一样直接读/写（甚至执行）那片 Flash，
    //无需再调用 read()/seek() 等系统调用
    //例如：把 U 盘里的电影 直接 拖到内存窗口里播放，而不是 每次读 1 MB 再播 1 MB
    //总量 2 MB（32 槽 × 64 KB），用光会返回 ESP_ERR_NO_FREE_PAGES
    ...
    //如果剩余的可用空间 比 分区大小 还小，就没法 mmap 了
    ...
    //把 assets 分区整片挂到 CPU 地址空间（只读）
    //成功后 mmap_root_ 就是可以直接解引用的首地址
    ...
    //对资源的名字，大小，偏移进行索引，保存在assets_index_中
    //这样的话可用GetAsset("alert.wav")直接获取资源而不用拷贝
